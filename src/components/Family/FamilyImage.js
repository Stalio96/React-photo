...
const FamilyImage = ({
    ...
    const length = familyState.length;

    const deleteHandler = (e) => {
        // e.preventDefault();
        
        photoService.deleteFamily(photo._id)
            .then(() => {
                console.log('deleted');
            })
    }
    ...
    const getDirection = (direction) => {
        if(direction == 'right'){
            ...
            setTempImg(familyState[way].imageUrl);
    ...
    return (
        <>
            ...
            <div data-aos={aos} data-aos-offset={aos_offset} className="familyEl" >
                <img className="familyImage" src={photo.imageUrl} style={{ width: '100%' }} onClick={() => getImg(photo.imageUrl)} />
                {
                    user.username
